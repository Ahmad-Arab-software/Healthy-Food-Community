...
// Stap 1: Haal de gelikede recepten op uit de database
$query = "SELECT id_meal FROM likes WHERE user_id = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();
$liked_meals = $result->fetch_all(MYSQLI_ASSOC);
$recipes = [];
// Controleer of de gebruiker gelikede recepten heeft
if (!empty($liked_meals)) {

    foreach ($liked_meals as $meal) {
        $meal_id = $meal['id_meal'];

        // Haal de recepten op van de API met behulp van het meal_id
        $api_url = "https://www.themealdb.com/api/json/v1/1/lookup.php?i={$meal_id}";
        $response = file_get_contents($api_url);
        $found_recipes = json_decode($response, true);

        if (!empty($found_recipes["meals"])) {
            $recipes = array_merge($recipes, $found_recipes["meals"]);
        }
    }
}
?>
...
<div class="container mx-auto mt-10 z-1">
    <h1 class="text-4xl font-bold mb-6">Favorite Recipes</h1>

    <?php if (!empty($recipes)): ?>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 text-black z-2">
            <?php foreach ($recipes as $recipe): ?>
                <div class="bg-white rounded-lg shadow-lg p-6 transform transition-transform hover:scale-105">

                    <img src="<?= $recipe['strMealThumb']; ?>" alt="<?= htmlspecialchars($recipe['strMeal']); ?>" class="w-full h-48 object-cover mb-4 rounded-md">
                    <h2 class="text-2xl font-bold mb-4"><?= htmlspecialchars($recipe['strMeal']); ?></h2>
                    <button class="view-recipe bg-[#178a0e] text-white py-2 px-4 rounded hover:bg-green-700" onclick="openRecipeDetails('<?= htmlspecialchars($recipe['idMeal']); ?>')">View recipe</button>
                </div>
            <?php endforeach; ?>
        </div>
    <?php else: ?>
        <p class="text-xl text-gray-600">You haven't liked any recipes yet.</p>
    <?php endif; ?>
</div>

<!-- Popup voor receptdetails -->
<section class="  container  mx-auto p-6">
    <section id="recipe-details-popup"
        ...
        <div
            ...
            <button id="dislike-recipe-btn" class="absolute mb-2 top-4 left-4 text-red-500 text-2xl">
                <i class="fa-solid fa-trash ml-2 mb-22 text-red-500 text-2xl"></i>
            </button>
...
<script src="../js/remove_recipe.js" defer></script>
